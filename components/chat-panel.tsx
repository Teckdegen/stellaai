      ...
      if (isLikelyClarityCode || hasCodeBlock) {
        ...
        if (codeBlockMatch) {
          ...
          if (code.includes("(define") || code.includes("(begin") || code.trim().startsWith("(")) {
            // Ensure we have a meaningful explanation (at least 50 words)
            let finalExplanation = explanation || "Code generated by Clarity AI";
            if (finalExplanation.split(/\s+/).length < 50) {
              // Generate a more detailed explanation
              finalExplanation = `I've analyzed your request and generated the following Clarity smart contract code. This implementation includes all the necessary functions and follows Stacks blockchain best practices. The contract has been structured to ensure proper error handling and efficient execution. You can review the code in the editor and deploy it when ready.`;
            }
            
            onCodeUpdate(code, finalExplanation)
            
            // Save code version to history
            ChatHistoryManager.saveCodeVersion(projectId, code, finalExplanation)
            ...
            if (explanation) {
              ...
              setMessages(prev => {
                ...
                const lastMsg = newMessages[newMessages.length - 1]
                if (lastMsg.id === lastMessage.id) {
                  lastMsg.content = finalExplanation
                }
                return newMessages
              })
            }
          ...
          // Generate a detailed explanation (at least 50 words)
          const explanation = `I've analyzed your request and generated the following Clarity smart contract code. This implementation includes all the necessary functions and follows Stacks blockchain best practices. The contract has been structured to ensure proper error handling and efficient execution. You can review the code in the editor and deploy it when ready.`;
          
          onCodeUpdate(content, explanation)
          
          // Save code version to history
          ChatHistoryManager.saveCodeVersion(projectId, content, explanation)
          
          // Update chat to show a detailed explanation instead of code
          setMessages(prev => {
            const newMessages = [...prev]
            const lastMsg = newMessages[newMessages.length - 1]
            if (lastMsg.id === lastMessage.id) {
          ...
          // If we detect Clarity patterns but can't extract code properly,
          // send the entire content to the editor and show a message in chat
