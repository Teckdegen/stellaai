...
(define-public (pause)
  (begin
    ...
    (var-set contract-paused true)
    (print {event: "contract-paused", by: tx-sender, block: block-height})
    (ok true)))
...
(define-public (unpause)
  (begin
    ...
    (var-set contract-paused false)
    (print {event: "contract-unpaused", by: tx-sender, block: block-height})
    (ok true)))
...
(define-public (trigger-emergency-stop)
  (begin
    (asserts! (or
      (is-eq tx-sender contract-owner)
    ...
    (print {event: "emergency-stop", by: tx-sender})
    (ok true)))

(define-public (cancel-emergency-stop)
  (begin
    (asserts! (is-eq tx-sender contract-owner) err-unauthorized)
    (var-set emergency-stop false)
    (ok true)))

(define-private (require-not-stopped)
...
(define-data-var circuit-breaker-level uint u0)
;; Level 0: Normal operation
;; Level 1: Limited functionality
;; Level 2: Emergency mode
;; Level 3: Complete shutdown

(define-public (set-circuit-breaker (level uint))
  (begin
    (asserts! (is-eq tx-sender contract-owner) err-unauthorized)
    (asserts! (<= level u3) err-invalid-level)
    (var-set circuit-breaker-level level)
    (print {event: "circuit-breaker-updated", level: level})
    (ok true)))

(define-private (require-level (max-level uint))
  (asserts! (<= (var-get circuit-breaker-level) max-level) err-circuit-breaker))

(define-public (normal-function)
  (begin
    (try! (require-level u0))
    (ok true)))

(define-public (limited-function)
  (begin
    (try! (require-level u1))
    (ok true)))

(define-public (emergency-function)
  (begin
    (try! (require-level u2))
    (ok true)))
\`\`\`

### 8.5 Reentrancy Protection
...
;; Good news: Clarity prevents reentrancy by design!
;; - No recursion allowed
;; - External calls must complete before continuing
;; - State changes are atomic

;; However, you should still follow checks-effects-interactions pattern

;; ❌ BAD PATTERN (vulnerable in other languages)
...
;; ============================================
;; MUTEX PATTERN (rarely needed in Clarity)
;; ============================================
...
(define-private (acquire-lock)
  (begin
    (asserts! (not (var-get locked)) err-locked)
    (var-set locked true)
    (ok true)))
...
\`\`\`clarity
;; ============================================
;; ERROR: Variable without initial value
;; ============================================

;; ❌ WRONG
(define-data-var counter uint)
;; Error: define-data-var expects 3 arguments, got 2
...
;; ❌ WRONG
(define-data-var amount uint false)
;; Error: expecting uint, found bool
...
;; ❌ WRONG
(define-public (use-undefined)
  (ok (var-get undefined-var)))
;; Error: undefined variable

;; ✅ CORRECT - Define before use
(define-data-var my-var uint u0)
(define-public (use-defined)
  (ok (var-get my-var)))

;; ============================================
;; ERROR: Public function not returning response
;; ============================================

;; ❌ WRONG
(define-public (bad-function)
  u100)
;; Error: expecting response type
...
;; ❌ WRONG
(define-constant owner "ST1PQHQKV...")
;; Error: expecting principal

;; ✅ CORRECT
(define-constant owner 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM)

;; ============================================
;; ERROR: List without max length
;; ============================================

;; ❌ WRONG
(define-data-var items (list uint) (list))
;; Error: list type requires max length

;; ✅ CORRECT
(define-data-var items (list 100 uint) (list))

;; ============================================
;; ERROR: String without max length
;; ============================================

;; ❌ WRONG
(define-data-var name (string-ascii) "hello")
;; Error: string-ascii requires max length

;; ✅ CORRECT
(define-data-var name (string-ascii 50) "hello")
\`\`\`
...
(define-public (overflow-test)
  (ok (+ u340282366920938463463374607431768211455 u1)))
...
(define-public (safe-add (a uint) (b uint))
  (let ((max-uint u340282366920938463463374607431768211455))
    (asserts! (<= (- max-uint a) b) err-overflow)
    (ok (+ a b))))
...
;; ❌ WILL FAIL at runtime
(define-public (divide-test (a uint) (b uint))
  (ok (/ a b)))
;; Runtime error if b is u0
...
;; ❌ WILL FAIL at runtime
(define-public (bad-unwrap (user principal))
  (ok (unwrap-panic (map-get? balances user))))
;; Runtime error if user not in map

;; ✅ CORRECT - Use unwrap! or default-to
(define-public (good-unwrap (user principal))
  (ok (unwrap! (map-get? balances user) err-not-found)))

(define-public (safe-get (user principal))
  (ok (default-to u0 (map-get? balances user))))
...
;; ❌ WILL FAIL at runtime
(define-public (bad-element-at)
  (ok (unwrap-panic (element-at (list u1 u2 u3) u10))))
;; Runtime error: index out of bounds

;; ✅ CORRECT - Check length first
(define-public (safe-element-at (items (list 100 uint)) (index uint))
  (begin
    (asserts! (< index (len items)) err-out-of-bounds)
    (ok (unwrap! (element-at items index) err-not-found))))
...
;; ❌ WILL FAIL at runtime
(define-public (bad-append (items (list 3 uint)))
  (let (
    (new-list (unwrap-panic (as-max-len? (append items u4) u3)))
  )
    (ok new-list)))
;; Runtime error: list exceeds max length

;; ✅ CORRECT - Check length before append
(define-public (safe-append (items (list 3 uint)) (item uint))
  (begin
    (asserts! (< (len items) u3) err-list-full)
    (ok (unwrap! (as-max-len? (append items item) u3) err-list-overflow))))
\`\`\`
...
\`\`\`clarity
;; ============================================
;; ERROR: Using contract-caller instead of tx-sender
;; ============================================

;; ❌ WRONG - Vulnerable to proxy attacks
(define-constant owner contract-caller)
;; owner will be the deployer contract, not the actual user!

;; ✅ CORRECT - Use tx-sender for ownership
(define-constant owner tx-sender)

;; When to use each:
(define-public (show-difference)
  (ok {
    tx-sender: tx-sender,          ;; Original transaction sender
    contract-caller: contract-caller  ;; Immediate caller (could be contract)
  }))
...
;; ❌ WRONG - No validation
(define-public (bad-transfer (recipient principal) (amount uint))
  (begin
    (try! (stx-transfer? amount tx-sender recipient))
    (ok true)))
;; Problems: allows zero amount, self-transfer, etc.

;; ✅ CORRECT - Full validation
(define-public (good-transfer (recipient principal) (amount uint))
  (begin
    (asserts! (> amount u0) err-invalid-amount)
    (asserts! (not (is-eq recipient tx-sender)) err-self-transfer)
    (asserts! (is-standard recipient) err-invalid-recipient)
...
;; ============================================
;; ERROR: Forgetting to update all related state
;; ============================================

;; ❌ WRONG - Inconsistent state
(define-public (bad-mint (amount uint))
  (begin
    (try! (ft-mint? my-token amount tx-sender))
    ;; Forgot to update total-supply!
    (ok true)))

;; ✅ CORRECT - Update all related state
(define-public (good-mint (amount uint))
  (begin
    (try! (ft-mint? my-token amount tx-sender))
    (var-set total-supply (+ (var-get total-supply) amount))
    (map-set last-mint-block tx-sender block-height)
    (ok true)))

;; ============================================
;; ERROR: Race conditions with allowances
;; ============================================

;; ❌ VULNERABLE - Allowance race condition
(define-public (bad-approve (spender principal) (amount uint))
  (begin
    ;; Directly setting allowance allows race condition
    (map-set allowances {owner: tx-sender, spender: spender} amount)
    (ok true)))

;; ✅ BETTER - Clear then set pattern
(define-public (safe-approve (spender principal) (amount uint))
  (begin
    ;; Option 1: Require zero first
    (asserts! (is-eq u0 (default-to u0 
      (map-get? allowances {owner: tx-sender, spender: spender}))) 
      err-must-clear-first)
    (map-set allowances {owner: tx-sender, spender: spender} amount)
    (ok true)))
...
;; ❌ WRONG ORDER
(define-public (wrong-order (recipient principal) (amount uint))
  (stx-transfer? tx-sender recipient amount))
;; WRONG! Order is: amount, sender, recipient

;; ✅ CORRECT ORDER
(define-public (correct-order (recipient principal) (amount uint))
  (stx-transfer? amount tx-sender recipient))

;; ============================================
;; ERROR: Not handling transfer response
;; ============================================

;; ❌ WRONG - Ignores response
(define-public (unhandled-transfer (recipient principal) (amount uint))
  (begin
    (stx-transfer? amount tx-sender recipient)  ;; Response ignored!
    (ok true)))

;; ✅ CORRECT - Handle response
(define-public (handled-transfer (recipient principal) (amount uint))
  (begin
    (try! (stx-transfer? amount tx-sender recipient))
    (ok true)))

;; ============================================
;; ERROR: Contract sending without as-contract
;; ============================================

;; ❌ WRONG - Contract can't send its STX
(define-public (bad-withdrawal (amount uint))
  (stx-transfer? amount tx-sender contract-owner))
;; This tries to send from USER, not contract!

;; ✅ CORRECT - Use as-contract
(define-public (good-withdrawal (amount uint))
  (as-contract (stx-transfer? amount tx-sender contract-owner)))

;; ============================================
;; ERROR: Using non-existent tx-sent
;; ============================================
...
(define-public (bad-send (recipient principal) (amount uint))
  (begin
    (try! (stx-transfer? amount tx-sender recipient))
    (ok true)))
;; Will fail at runtime if insufficient balance

;; ✅ CORRECT - Check balance first
(define-public (good-send (recipient principal) (amount uint))
  (begin
...
(define-map deployed-contracts uint principal)
(define-data-var contract-count uint u0)

(define-public (deploy-contract (code (buff 1048576)))
  (let (
    (contract-id (var-get contract-count))
    ;; Note: Actual deployment would use different mechanism
  )
    (map-set deployed-contracts contract-id tx-sender)
    (var-set contract-count (+ contract-id u1))
    (print {event: "contract-deployed", id: contract-id, deployer: tx-sender})
    (ok contract-id)))

(define-read-only (get-contract (id uint))
  (ok (map-get? deployed-contracts id)))

(define-read-only (get-total-contracts)
  (ok (var-get contract-count)))

;; ============================================
;; CLONE PATTERN (Minimal Proxy)
;; ============================================

(define-map clones uint {
  implementation: principal,
  owner: principal,
  data: (buff 1024)
})

(define-data-var clone-count uint u0)

(define-public (create-clone (implementation principal) (init-data (buff 1024)))
  (let ((clone-id (var-get clone-count)))
    (map-set clones clone-id {
      implementation: implementation,
      owner: tx-sender,
      data: init-data
    })
    (var-set clone-count (+ clone-id u1))
    (ok clone-id)))
\`\`\`

### 10.2 Upgradeable Pattern
...
(define-constant proxy-admin tx-sender)

(define-public (upgrade (new-implementation principal))
  (begin
    (asserts! (is-eq tx-sender proxy-admin) err-unauthorized)
...
(define-map permanent-storage principal uint)

(define-public (set-value (user principal) (value uint))
  (begin
    (asserts! (is-eq tx-sender contract-owner) err-unauthorized)
    (map-set permanent-storage user value)
    (ok true)))

(define-read-only (get-value (user principal))
  (ok (map-get? permanent-storage user)))
\`\`\`
...
(define-constant test-user-1 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM)
(define-constant test-user-2 'ST2PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRT45A2A)

;; Setup function for tests
(define-public (test-setup)
  (begin
    (map-set balances test-user-1 u1000000)
    (map-set balances test-user-2 u500000)
    (ok true)))

;; Reset function for tests
(define-public (test-reset)
  (begin
    (map-delete balances test-user-1)
    (map-delete balances test-user-2)
    (var-set total-supply u0)
    (ok true)))

;; Assertion helpers
(define-private (assert-eq (actual uint) (expected uint) (message (string-ascii 100)))
  (asserts! (is-eq actual expected) 
    (err (concat "Assertion failed: " message))))

(define-private (assert-true (condition bool) (message (string-ascii 100)))
  (asserts! condition 
    (err (concat "Assertion failed: " message))))
...
(define-public (run-all-tests)
  (begin
    (try! (test-transfer-function))
    (try! (test-balance-function))
    (try! (test-edge-cases))
    (ok "All tests passed")))
\`\`\`
...
(define-read-only (inspect-contract-state)
  (ok {
    total-supply: (var-get total-supply),
    contract-paused: (var-get contract-paused),
    user-count: (var-get user-count),
    admin-count: (len (map keys admins))
  }))

(define-read-only (inspect-user-state (user principal))
  (ok {
    balance: (default-to u0 (map-get? balances user)),
    is-admin: (default-to false (map-get? admins user)),
    last-update: (default-to u0 (map-get? last-update-block user))
  }))
\`\`\`
...
### 12.1 Efficient Data Structures
...
;; ❌ INEFFICIENT - Multiple map accesses
(define-public (bad-balance-check (user principal))
  (ok (and 
    (is-some (map-get? balances user))
    (> (default-to u0 (map-get? balances user)) u0))))

;; ✅ EFFICIENT - Single map access
(define-public (good-balance-check (user principal))
  (let ((balance (map-get? balances user)))
    (ok (and 
      (is-some balance)
      (> (unwrap! balance u0) u0)))))
...
(define-public (bad-batch-transfer (recipients (list 10 principal)) (amounts (list 10 uint)))
  (begin
    (map transfer-to recipients amounts)  ;; Creates intermediate list
    (ok true)))

;; ✅ EFFICIENT - Direct iteration
(define-public (good-batch-transfer (recipients (list 10 principal)) (amounts (list 10 uint)))
  (begin
    (asserts! (is-eq (len recipients) (len amounts)) err-length-mismatch)
    (fold transfer-pair (zip recipients amounts) true)
    (ok true)))

(define-private (transfer-pair (pair {recipient: principal, amount: uint}) (acc bool))
  (begin
    (try! (stx-transfer? (get amount pair) tx-sender (get recipient pair)))
    acc))

(define-private (zip (list1 (list 10 principal)) (list2 (list 10 uint)))
  ;; Implementation would create pairs of elements
  (list))
\`\`\`
...
(define-data-var last-calculated-block uint u0)
(define-data-var cached-result uint u0)

(define-read-only (expensive-calculation)
  (if (is-eq (var-get last-calculated-block) block-height)
    ;; Return cached result
    (ok (var-get cached-result))
    ;; Calculate and cache new result
    (let ((result (perform-expensive-calculation)))
      (var-set last-calculated-block block-height)
      (var-set cached-result result)
      (ok result))))

(define-private (perform-expensive-calculation)
...
;; ❌ INEFFICIENT - Always processes entire list
(define-read-only (bad-validation (items (list 100 uint)))
  (ok (and 
    (> (len items) u0)
    (is-eq (fold + (map is-valid items) u0) (len items)))))

;; ✅ EFFICIENT - Early termination with fold
(define-read-only (good-validation (items (list 100 uint)))
  (ok (fold validate-item items true)))

(define-private (validate-item (item uint) (acc bool))
  (and acc (is-valid item)))

(define-private (is-valid (item uint))
  (> item u0))
\`\`\`
...
;; ✅ Configuration constants have appropriate values
;; ✅ Principal constants are valid addresses

;; 2. Storage Validation
;; ✅ All data variables have initial values
;; ✅ Map keys and values are properly typed
;; ✅ No unused storage variables

;; 3. Function Validation
;; ✅ All public functions return (ok ...) or (err ...)
;; ✅ Private functions don't return responses unless needed
;; ✅ Read-only functions don't modify state

;; 4. Security Validation
;; ✅ Owner checks on administrative functions
...
(define-read-only (verify-deployment-readiness)
  (ok {
    constants-valid: true,        ;; Manual verification required
    storage-valid: true,          ;; Manual verification required
    functions-valid: true,        ;; Manual verification required
    security-valid: true,         ;; Manual verification required
    gas-optimized: true,          ;; Manual verification required
    testing-complete: true,       ;; Manual verification required
    documentation-complete: true  ;; Manual verification required
  }))
\`\`\`

## 14. Complete Code Examples

### 14.1 Token Contract Example
...
;; Contract: MyToken
;; Version: 1.0.0
;; Description: Standard token implementation with all essential features
...
(define-constant initial-supply u1000000000000000) ;; 1 billion tokens
...
;; EVENTS
;; ============================================
;; Events are printed as tuples for off-chain monitoring
...
(define-private (transfer-tokens (sender principal) (recipient principal) (amount uint))
  (let (
    ...
    ;; Update balances
    (if (is-eq amount sender-balance)
...
(define-public (mint (amount uint))
  (begin
    ...
    (let ((owner-balance (default-to u0 (map-get? balances contract-owner))))
      (map-set balances contract-owner (+ owner-balance amount)))
    
    ;; Emit events
    (print {event: "mint", to: contract-owner, amount: amount})
    (print {event: "total-supply-update", new-total: (var-get total-supply)})
...
(define-public (burn (amount uint))
  (begin
    ...
    (let ((sender-balance (default-to u0 (map-get? balances tx-sender))))
      ...
      ;; Emit events
      (print {event: "burn", from: tx-sender, amount: amount})
      (print {event: "total-supply-update", new-total: (var-get total-supply)})
...
(begin
  ...
  (print {event: "initial-mint", to: contract-owner, amount: initial-supply}))
\`\`\`

### 14.2 NFT Contract Example

\`\`\`clarity
;; ============================================
;; COMPLETE NFT CONTRACT (SIP-009 COMPLIANT)
;; ============================================

;; Contract: MyNFT
;; Version: 1.0.0
;; Description: Standard NFT implementation following SIP-009

;; ============================================
;; CONSTANTS
;; ============================================
(define-constant contract-owner tx-sender)
(define-constant nft-name "MyNFT Collection")
(define-constant nft-symbol "MNFT")
...
(define-private (is-token-owner (token-id uint) (user principal))
  (is-eq (default-to 'SP000000000000000000002Q6VF78 (map-get? token-owners token-id)) user))

(define-private (burn-token (token-id uint))
  (begin
    (map-delete token-owners token-id)
    (map-delete token-metadata token-id)
    (map-delete token-approvals token-id)
    (var-set total-supply (- (var-get total-supply) u1))
    (print {event: "burn", token-id: token-id, owner: tx-sender})))
...
(define-public (mint (name (string-ascii 100)) (description (string-utf8 1000)) (uri (string-ascii 256)))
  (begin
    (asserts! (is-contract-owner) err-unauthorized)
    (asserts! (is-not-paused) err-contract-paused)
    ...
    (let (
      ...
      ;; Set token owner
      (map-set token-owners new-token-id contract-owner)
      ...
      ;; Emit events
      (print {event: "mint", token-id: new-token-id, owner: contract-owner})
      (print {event: "total-supply-update", new-total: (var-get total-supply)})
      
      (ok new-token-id))))

(define-public (transfer (to principal) (token-id uint))
  (begin
    (asserts! (is-not-paused) err-contract-paused)
    
    (let (
      (current-owner (default-to 'SP000000000000000000002Q6VF78 (map-get? token-owners token-id)))
      (approved-address (map-get? token-approvals token-id))
      (is-operator-approved (default-to false 
        (map-get? operator-approvals {owner: current-owner, operator: tx-sender})))
    )
      ;; Check authorization
      (asserts! (or 
        (is-eq tx-sender current-owner)
        (is-eq tx-sender approved-address)
        is-operator-approved) err-not-token-owner)
      ...
      ;; Emit transfer event
      (print {event: "transfer", from: current-owner, to: to, token-id: token-id})
...
(define-public (approve (operator principal) (token-id uint))
  (begin
    ...
    (let (
      ...
      ;; Emit approval event
      (print {event: "approval", owner: current-owner, approved: operator, token-id: token-id})
...
;; This contract implements the SIP-009 NFT trait
;; Functions get-last-token-id, get-token-uri, and get-owner are implemented above
\`\`\`
